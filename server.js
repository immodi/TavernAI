...
app.use((req, res, next) => {
  if (
    req.url.startsWith("/User%20Avatars/") &&
    is_colab &&
    process.env.googledrive == 2
  ) {
    let requestUrl = url.parse(req.url);
...
app.use("/cardeditor", (req, res) => {
  ...
  const filePath = decodeURIComponent(
    path.join(
      process.cwd(),
      "public/cardeditor",
      requestUrl.pathname.replace(/%20/g, " ")
    )
  );
  fs.readFile(filePath, (err, data) => {
    if (err) {
...
app.post("/generate_webui", jsonParser, function (request, response_generate) {
  ...
  client
    .post(api_server + "/v1/completions", args, function (data, response) {
      if (response.statusCode == 200) {
        console.log(data);
        response_generate.send(data);
      }
...
app.post(
  "/tokenizer_webui",
  jsonParser,
  function (request, response_tokenizer) {
    if (!request.body) return response_generate.sendStatus(400);
    //console.log(request.body.prompt);
    //const dataJson = json5.parse(request.body);
    //console.log(request.body);
...
app.post("/savechat", jsonParser, function (request, response) {
  ...
  var dir_name = String(request.body.card_filename).replace(
    `.${characterFormat}`,
    ""
  );
...
app.post("/changechatname", jsonParser, function (request, response) {
  try {
    let dir_name = String(request.body.character_filename).replace(
      `.${characterFormat}`,
      ""
    );
    let filePath =
      chatsPath + dir_name + "/" + request.body.chat_filename + ".jsonl";
    //read
...
app.post(
  "/getstatus_webui",
  jsonParser,
  function (request, response_getstatus) {
    if (!request.body) return response_getstatus.sendStatus(400);
    ...
    client
      .get(
        api_server + "/v1/internal/model/info",
        args,
        function (data, response) {
          if (response.statusCode == 200) {
            if (data.result != "ReadOnly") {
...
app.post(
  ...
  async function (request, response) {
    ...
    if (!fs.existsSync(charactersPath + target_img + `.${characterFormat}`)) {
      if (!fs.existsSync(chatsPath + target_img))
        fs.mkdirSync(chatsPath + target_img);
...
app.post("/createroom", urlencodedParser, async function (request, response) {
  ...
  if (!fs.existsSync(roomsPath + target_file + fileExtension)) {
    if (!scenario) await roomWrite(target_file, characterFiles);
    else
      await roomWrite(
        target_file,
        characterFiles,
...
app.post("/editroom", urlencodedParser, async function (request, response) {
  try {
    ...
    let merged_room_metadata = Object.assign(
      {},
      old_room_metadata,
      new_room_metadata
    );
    await roomWrite(
      filename,
      merged_room_metadata.character_files,
...
async function roomWrite(
  ...
  try {
    ...
    let createDate = create_date ? create_date : Date.now();
    // let characterNamesArray = characterNames.isArray() ? characterNames : [characterNames];
    if (!Array.isArray(characterFiles)) characterFiles = [characterFiles]; // Convert character names into an array if not already (happens when user selected only one character for a room)
    ...
    for (const item of imgFiles) {
      ...
      try {
        ...
        if (error instanceof SyntaxError) {
          console.error(
            "Character info from index " + i + " is not valid JSON!",
            error
          );
        } else {
          console.error(
...
app.post("/deleteworld", jsonParser, function (request, response) {
  ...
  const path =
    worldPath + request.body.world_name.replace(/\.\.[\/\\]/g, "") + ".json";
  fs.stat(path, function (err, stat) {
    if (err === null) {
      fs.rm(path, () => {});
      response.send({ result: "ok" });
...
app.post("/delbackground", jsonParser, function (request, response) {
  if (!request.body) return response.sendStatus(400);
  rimraf(
    "public/backgrounds/" + request.body.bg.replace(/\.\.[\/\\]/g, ""),
    (err) => {
      if (err) {
        response.send(err);
        return console.log(err);
...
app.post("/savestyle", jsonParser, function (request, response) {
  ...
  if (this_style != "classic.css") {
    file_data =
      '@import "../templates/classic.css";@import "../templates/' +
      this_style +
      '";';
  }
...
app.post(
  "/generate_horde",
  jsonParser,
  function (request, response_generate_horde) {
    hordeActive = true;
    hordeData = null;
    hordeError = null;
    hordeQueue = 0;
    ...
    var args = {
      ...
      headers: {
        "Content-Type": "application/json",
        apikey: request.body.horde_api_key,
      },
      requestConfig: {
        timeout: 10 * 600 * 1000,
      },
    ...
    client
      .post(
        api_horde + "/v2/generate/text/async",
        args,
        function (data, response) {
...
function pollHordeStatus(id, args, callback) {
  return new Promise((resolve, reject) => {
    client
      .get(
        api_horde + "/v2/generate/text/status/" + id,
        args,
        function (gen, response) {
          ...
          if ((gen.done || gen.finished) && gen.generations != undefined) {
            ...
            gen.faulted ||
            (!gen.processing && !gen.waiting && !gen.is_possible)
          ) {
            hordeActive = false;
            hordeQueue = 0;
...
app.post(
  "/getstatus_openai",
  jsonParser,
  function (request, response_getstatus_openai) {
    ...
    client
      .get(api_url_openai + "/models", args, function (data, response) {
        if (response.statusCode == 200) {
...
app.post(
  ...
  function (request, response_generate_openai) {
    ...
    client
      .post(api_url_openai + request_path, args, function (data, response) {
        try {
          if (isChatModel(request.body.model)) {
            console.log(data);
...
app.post(
  "/getstatus_claude",
  jsonParser,
  function (request, response_getstatus_claude) {
...
app.post(
  "/generate_claude",
  jsonParser,
  function (request, response_generate_claude) {
        ...
        function (data, response) {
          try {
            ...
            if (response.statusCode == 400) {
              ...
              return response_generate_claude.send({
                error: true,
                error_code: 400,
                error_message:
    ...
    if (filedata) {
      if (format == "json") {
        ...
        try {
          ...
          if (jsonData.spec === "chara_card_v2")
            jsonData = parseV2CharacterCard(jsonData);
          img_name = setCardName(jsonData.name);
...
app.listen(server_port, listenIp, function () {
  ...
  clearUploads();
  initCardeditor();
  const autorunUrl = new URL("http://" + "127.0.0.1" + (":" + server_port));
  if (autorun) open(autorunUrl.toString());
  console.log(
    "TavernAI has started and is available on IP: 127.0.0.1 at PORT: " +
      server_port
  );
          ...
          if (fs.existsSync(targetPath)) {
            ...
            fs.unlink(filePath, (err) => {
              if (err) {
                ...
                console.log(
                  targetName + " has been converted to ." + this_format
                );
